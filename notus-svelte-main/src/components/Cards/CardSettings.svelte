<script>
  ...
  const response = await fetch(API+'/upload', {
    ...
    .then((data) => {
        img_cid = data.cid;
        return StudentContract.setprofpichash(data.cid,{from: connectedAccount});
  ...
  const response = await fetch(API+'/upload', {
    ...
    .then((data) => {
        data_cid = data.cid;
        return StudentContract.setabouthash(data.cid,{from: connectedAccount});
    
    })
    let msg = StudentContract.addDetails(firstname,city,emailaddress,{from: connectedAccount});
}
</script>
